...
def on_page_markdown(markdown: str, *, page: Page, config: MkDocsConfig, files: Files):
    ...
    def replace(match: Match):
        ...
        if badge_type == "version":
            return _badge_for_version(args, page, files, config.repo_name)
...
def _badge_for_flags(arg, page: Page, files: Files):
    ...
    if arg == "switch":
        return _badge_for_switch(page, files)
...
def _badge_for_version(text: str, page: Page, files: Files, repo: str | None):
    icon = "material-tag-outline"
    href = f"https://github.com/{repo or "cpp-linter/cpp-linter"}/releases/" + (
...
# Create badge for experimental flag
def _badge_for_switch(page: Page, files: Files):
    icon = "material-flag"
    return _badge(icon=f":{icon}:", text="takes no value")
